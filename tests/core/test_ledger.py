...
def test_facts_assigns_sequential_indices_ignoring_other_events() -> None:
    session = _session()
    ...
    assert facts(session) == [
        Fact(index=0, content="a-content", source="read_file"),
        Fact(index=1, content="ok", source="shell"),
    ]
